...
public class CropTestController {
	...
	public void setUp() {
		...
		cropDto = new CropDto(101,"Fruit","apple","10kg","724971714141","mandrup","goa","pune",411028);
	...
	void getCropByUserId() throws Exception{
		when(cropService.getbyuserId(crop.getUserId())).thenReturn(cropList);
		mockMvc.perform(MockMvcRequestBuilders.get("/crop/getbyuserId/1002")
				.contentType(MediaType.APPLICATION_JSON)
				.content(asJsonString(crop)))
				.andDo(MockMvcResultHandlers.print());
				verify(cropService,times(1)).getbyuserId(1002);
	}
	
	@Test
	void updateCropTest() throws Exception{
		when(cropService.UpdateCrop(cropDto)).thenReturn("updated");
		mockMvc.perform(MockMvcRequestBuilders.put("/crop/update")
				.contentType(MediaType.APPLICATION_JSON)
				.content(asJsonString(crop)))
				.andDo(MockMvcResultHandlers.print());
				verify(cropService,times(1)).UpdateCrop(cropDto);
	}
